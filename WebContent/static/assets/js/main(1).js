$(document).ready(function() {
    ...
    $(v).mousewheel(function(U, X, S, R) {
        ...
        if (l || k || i) {
            W = (R > 0 || X > 0) ? true: false;
            Q = (R < 0 || X < 0) ? true: false
        }
        if (W) {
            if (O === 0) {
                T = false
    ...
    function P() {
        ...
        w = typeof Q.innerWidth == "undefined" ? R.clientWidth: Q.innerWidth();
    ...
    z.each(function(Q, S) {
        ...
        if (Q == 0) {
            if (!L[Q]["bgInit"]) {
                J(B + "img/" + (l ? "ie6/pic_1_1.jpg": "pic_1_2.jpg"),
                function(Y) {
                    L[Q]["jqo"].fadeOut(0);
                    var Z = {
            ...
            if (!L[Q]["imgInit"] && !l) {
                L[Q]["imgInit"] = true;
                L[Q]["jqImg"].each(function() {
                    var Y = $(this);
                    var X = Y.attr("data-bg");
                    J(B + "img/section" + (Q + 1) + "/" + X,
                    function(Z) {
                        Y.css("background-image", "url(" + Z + ")");
    ...
    function I(Q, S) {
        ...
        if (!L[Q]["bgInit"]) {
            ...
            J(B + "img/" + (l ? "ie6": "modern") + "/" + R + ".jpg",
            function(T) {
                L[Q]["jqo"].find(".inner").css("background-image", "url(" + T + ")")
            })
        }
        if (!L[Q]["imgInit"] && !l) {
            L[Q]["imgInit"] = true;
            L[Q]["jqImg"].each(function() {
                ...
                J(B + "img/section" + (Q + 1) + "/" + T,
                function(V) {
                    U.css("background-image", "url(" + V + ")");
    ...
    function e() {
        ...
        T.css({
            width: 554 * S + "px",
            height: 176 * S + "px",
            "margin-left": -554 * (S - 1) / 2 + "px",
            "margin-top": -176 * (S - 1) / 2 + "px",
            opacity: "0"
        });
        R.hide();
        T.show();
    ...
    function t() {
        ...
        if (typeof O == "number") {
            E.eq(O).removeClass("item-active")
        ...
        O = Q;
        if (!M) {
            location.hash = "#node=" + O;
            M = false
        }
    }
    K.bind("click",
    function(S) {
    	if(totalFlag){/*修改标记，判断是否执行完动作，如果为执行完，当点击右侧选项时，不执行任何动作*/
    		return;
    	}
        var R = S.target;
        while (!R.className || (R.className != "navigation" && R.className.indexOf("item") == -1)) {
            R = R.parentNode
        }
        var Q = R.getAttribute("data-for");
        if (Q) {
            f = true;
            p(Q)
        }
                ...
                if (R == 9) {
                    ...
                    W.each(function(ae) {
                        ...
                        var af = {
                            dom: ag,
                            sectionHeight: ac,
                            positionTop: parseInt(ag.css("top"), 10),
                            topBorderPos: Z,
                            speed: 0.5
                        };
                        q(af)
    ...
    function u(V) {
        ...
        q({
            dom: Q,
            sectionHeight: T,
            positionTop: parseInt(Q.css("top"), 10),
            topBorderPos: R,
            speed: 1
        });
        q({
            dom: X,
            sectionHeight: T,
            positionTop: parseInt(X.css("top"), 10),
            topBorderPos: R,
            speed: 2
        });
        q({
            dom: U,
            sectionHeight: T,
            positionTop: parseInt(U.css("top"), 10),
            topBorderPos: R,
            speed: 4
        });
        q({
            dom: S,
            sectionHeight: T,
            positionTop: parseInt(S.css("top"), 10),
            topBorderPos: R,
            speed: 0.1
        })
    }
    ...
    if (!window.Browser) {
        ...
        if (!Browser._isInited) {
            ...
            Browser.hasFlash = !!k;
            Browser.flashVersion = k;
            Browser._isInited = true
        }
    }
    function f(o, n) {
        ...
        if (String.prototype.trim && !n) {
            o = String.prototype.trim.call(o)
        } else {
            var p = n ? (/(?:^[ \t\n\r]+)|(?:[ \t\n\r]+$)/g) : (/(^(\s|　)+)|((\s|　)+$)/g);
            o = o.replace(p, "")
        }
    ...
    function a() {
        var n;
        n = b.match(/MSIE ([^;]*)|Trident.*; rv(?:\s|:)?([0-9.]+)/);
        if (n && (n[1] || n[2])) {
            i = "MSIE";
            g = n[1] || n[2];
            return
    ...
    function m() {
        ...
        if (i === "MSIE") {
            ...
            if (q) {
                k = q.getVariable("$version").split(" ")[1]
            }
        } else {
            if (navigator.plugins && navigator.plugins.length > 0) {
                q = navigator.plugins["Shockwave Flash"];
                if (q) {
                    var p = q.description.split(" ");
                    for (var o = 0,
                    n = p.length; o < n; o++) {
                        if (Math.floor(p[o]) > 0) {
                            k = p[o];
